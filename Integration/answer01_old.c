...
double simpson_numerical_integration(integrand intg_arg)
{
	...
	integral = (1 / (2 * L)) * (delta_x / 3) * (integral) ; 
...

{
	...
	delta_x = (upper_limit - lower_limit) / (2 * n_intervals) ;
	
	value_x = lower_limit ;
	double cos_sin_value = cos_sin((n * M_PI * value_x) / L) ;
        integral = intg_arg.func_to_be_integrated(value_x) * (cos_sin_value) ;	
	...
	for(; cnt < i; cnt++) {
		value_x += delta_x ;
 		if (cnt % 2 == 0) {
			value_fcn = 2 * intg_arg.func_to_be_integrated(value_x) * (cos_sin((n * M_PI * value_x) / L)) ;
			integral = integral + value_fcn ;
		}
		else if (cnt % 2 == 1) {	
			value_fcn = 4 * intg_arg.func_to_be_integrated(value_x) * (cos_sin((n * M_PI * value_x) / L)) ;
			integral = integral + value_fcn ;
		}
		else if (cnt == (i - 1))
		{
	 		value_fcn = intg_arg.func_to_be_integrated(value_x) * (cos_sin((n * M_PI * value_x) / L)) ;
                        integral = integral + value_fcn ;	
		} 
	}
	//integral = integral - (value_fcn / 2) ;
	integral = (1 / L) * (delta_x / 3) * (integral) ; 
	return integral;
}
...
void fourier_coefficients(fourier fourier_arg) 
{
   int i;
   	for (i = 0; i < fourier_arg.n_terms; i++) {
		...
		if (i == 0) {	
			fourier_arg.a_i[i] = simpson_numerical_integration(fourier_arg.intg) ;
		}
		else {
			fourier_arg.a_i[i] = simpson_numerical_integration_for_fourier(fourier_arg.intg, i, cos) ; 
   			fourier_arg.b_i[i] = simpson_numerical_integration_for_fourier(fourier_arg.intg, i, sin) ;
		}
	}
