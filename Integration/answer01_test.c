                                                 ...
                                                 double (*cos_sin)(double))
{
	...
	delta_x = (upper_limit - lower_limit) / (n_intervals) ;
 	//double L = 0.0 ;	
	int cnt = 0 ;
	int i = n_intervals ;
	...
	for (; cnt < i; cnt++) {
	//	L = (delta_x / 2) ;
		value_fcn = 		      (delta_x / 6) * (((intg_arg.func_to_be_integrated(lower_limit)) * 
								(cos_sin((n * M_PI * lower_limit) / L)))  + 
								(4 * ((intg_arg.func_to_be_integrated((lower_limit + lower_limit + delta_x) / 2)) *
								(cos_sin((n * M_PI * (lower_limit +  lower_limit + delta_x) / 2) / L)))) + 
								((intg_arg.func_to_be_integrated(lower_limit + delta_x)) *
								(cos_sin((n * M_PI * (lower_limit + delta_x) / L))))) ;
		lower_limit += delta_x ;
		integral += value_fcn ;
	}
 //	L = (upper_limit - lower_limit) / 2 ;		
	integral = (1 / L) * (integral) ; 
   return integral;
}
// do not change this
#endif /* NTEST_SIMPSON */
...
void fourier_coefficients(fourier fourier_arg) 
{
   int i;
   	for (i = 0; i < fourier_arg.n_terms; i++) {
		...
		//	printf("%f", fourier_arg.a_i[i]) ;
		//	printf("%f", fourier_arg.b_i[i]) ;
		}
	}
}
#endif /* NTEST_FOURIER */
