...
static int Path_helper(Maze *maze, int curr, int curc, int endr, int endc, int count, distance *dist)
{	
	if (((dist->coordinate[curr][curc] == 0) && Is_path(maze, curr, curc)) || (dist->coordinate[curr][curc] > count)) {
	...
	maze->maze_array[curr][curc] = VISITED ;	
   	...
   	if (Is_path(maze, curr-1, curc)) {
	      	found = Path_helper(maze, curr-1, curc, endr, endc, count+1, dist);
   	...
   	if (Is_path(maze, curr, curc+1)) {
	      	found = Path_helper(maze, curr, curc+1, endr, endc, count+1, dist);
		if ((Is_path(maze, curr-1, curc)) || (Is_path(maze, curr+1, curc)) || (Is_path(maze, curr, curc-1)) || (Is_path(maze, curr, curc+1))) {
			found = -1 ;
		}	    
	  	if (found != -1) {
...
int Direction_creator(Maze *maze, int row, int col, int row_end, int col_end, distance *dist, FILE *dfptr) 
{
	int i, j ;
	i = 0 ;
	j = 0 ;
	...
	while (max > 1) {
		if (row < row_end) {
			if (dist->coordinate[row_end-1][col_end] == (max - 1)) {
				temp[max-1] = 'S' ; 			
				row_end = row_end - 1 ;
				max = max - 1 ;		
			}
			else if (dist->coordinate[row_end+1][col_end] == (max - 1)) {
				temp[max-1] = 'N' ;
				row_end = row_end + 1 ;
				max = max - 1 ;		
			}
			else if (dist->coordinate[row_end][col_end+1] == (max - 1)) {
				temp[max-1] = 'W' ;
				col_end = col_end + 1 ;
				max = max - 1 ;		
			}
                        ...
                        Coord source, Coord destination)
{
	...
	int steps = Path_helper(maze, source.row, source.col, destination.row, destination.col, 0, &dist); 

	int count = (dist.coordinate[destination.row][destination.col]) - 1;

	int max = Direction_creator(maze, source.row, source.col, destination.row, destination.col, &dist, dirc_file) ;
	fclose(dirc_file) ;
	max++ ;
	steps++ ;
